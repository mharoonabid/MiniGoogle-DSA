...
g++ -o forwardIndex backend/cpp/forwardIndex.cpp backend/cpp/config.cpp -std=c++17 || { echo -e "${RED}Forward index compilation failed.${RESET}"; exit 1; }
./forwardIndex || { echo -e "${RED}Forward index run failed.${RESET}"; exit 1; }

echo -e "${BLUE}=== Building Inverted Index ===${RESET}"
g++ -o invertedIndex backend/cpp/invertedIndex.cpp backend/cpp/config.cpp -std=c++17 || { echo -e "${RED}Inverted index compilation failed.${RESET}"; exit 1; }
./invertedIndex || { echo -e "${RED}Inverted index run failed.${RESET}"; exit 1; }
