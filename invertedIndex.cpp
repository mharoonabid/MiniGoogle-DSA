...
int main() {
    ...
    cout << "======================================" << endl;
    cout << "  INVERTED INDEX BUILDER" << endl;
    cout << "======================================" << endl;
    cout << "Program start: " << system_clock::to_time_t(system_clock::now()) << endl;
    cout << endl;
    
    InvertedIndex invertedIndex;
    
    // Build from forward index
    string forwardIndexPath = "C:/Users/dell/Documents/VSCode Code/MiniGoogle-DSA/Forward Index/forward_index_FULL.txt";
    invertedIndex.buildFromForwardIndex(forwardIndexPath);
    
    // Print statistics
    invertedIndex.printStatistics();
    
    // Save inverted index
    string outputPath = "C:/Users/dell/Documents/VSCode Code/MiniGoogle-DSA/Inverted Index/inverted_index_FULL.txt";
    invertedIndex.saveToFile(outputPath);
    
    auto programEnd = high_resolution_clock::now();
    auto totalProgramTime = duration_cast<seconds>(programEnd - programStart).count();
    
    cout << "\n======================================" << endl;
    cout << "  PROGRAM COMPLETE" << endl;
    cout << "======================================" << endl;
    cout << "Total program time: ";
    
    if (totalProgramTime < 60) {
        cout << totalProgramTime << " seconds" << endl;
    } else if (totalProgramTime < 3600) {
        cout << (totalProgramTime / 60) << " min " << (totalProgramTime % 60) << " sec" << endl;
    } else {
        int hours = totalProgramTime / 3600;
        int mins = (totalProgramTime % 3600) / 60;
        cout << hours << " hr " << mins << " min" << endl;
    }
    
    // Test search functionality
    cout << "\n=== Testing Search ===" << endl;
    cout << "Enter a lemma ID to search (or -1 to exit): ";
    
    int testLemma;
    while (cin >> testLemma && testLemma != -1) {
        cout << "\nSearching for lemma " << testLemma << "..." << endl;
        vector<string> results = invertedIndex.search(testLemma);
        
        cout << "Lemma " << testLemma << " found in " << results.size() << " documents" << endl;
        
        if (results.size() > 0) {
            cout << "First 10 documents: ";
            for (size_t i = 0; i < min(results.size(), size_t(10)); i++) {
                cout << results[i];
                if (i < min(results.size(), size_t(10)) - 1) cout << ", ";
            }
            cout << endl;
            
            double idf = invertedIndex.calculateIDF(testLemma);
            cout << "IDF score: " << fixed << setprecision(4) << idf << endl;
        }
        
        cout << "\nEnter another lemma ID (or -1 to exit): ";
    }
